...
func insert(args insertArgs) error {
	...
	if !isFolder {
		...
		book, err = lib.AddBook(URL.String, options)
		if err != nil {
			return err
		}
	} else {
		...
		book, err = lib.AddFolder(name, options)
		if err != nil {
			return err
		}
	}
	...
	if storeParentID {
		args.vars[parentIdKey] = lib.NullStringFromPtr(book.ParentID)
	}
	...
	if storeURL {
		args.vars[URLKey] = lib.NullStringFromPtr(book.URL)
	}

	if storeDesc {
		args.vars[descKey] = lib.NullStringFromPtr(book.Description)
	}
...
func handleNullString(vars map[string]interface{}, val string) (res lib.NullString, store bool, key string, err error) {
	if isVariableSet(val) {
		store = true
		key = stripVariable(val)
		return
		...
		if converted, ok := val.(lib.NullString); ok {
			res = converted
			return
		} else if converted, ok := val.(string); ok {
			res = lib.NullStringFrom(converted)
		} else {
			err = fmt.Errorf("Could not convert variable to string; type was %T", val)
		}
	} else if val != "NULL" {
		res = lib.NullStringFrom(val)
	}
...
func variableToString(value interface{}) (string, error) {
	...
	} else if v, ok := value.(lib.NullString); ok {
		if v.Valid {
			return v.String, nil
		} else {
			return "NULL", nil
...
func markDirty(expected []cmd.BookDTO, actual []cmd.BookDTO) {
	for i := range expected {
		...
		expected[i].ParentName = lib.NullString{}
	}
	...
	for i := range actual {
		...
		actual[i].ParentName = lib.NullString{}
	}
}
