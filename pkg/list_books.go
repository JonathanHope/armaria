...
func (o *listBookOptions) WithOrder(order Order) *listBookOptions {
	o.Order = order
	return o
}

// WithDirection sets the direction to order by.
func (o *listBookOptions) WithDirection(direction Direction) *listBookOptions {
...
func ListBooks(options *listBookOptions) ([]Book, error) {
	...
	return db.QueryWithDB(options.DB, config.DB, func(tx db.Transaction) ([]Book, error) {
		...
		if err != nil {
			return nil, fmt.Errorf("error while listing bookmarks: %w", err)
		}
